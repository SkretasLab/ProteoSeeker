...
### Phobius
<p align="justify">For either case of installation process, in order to use the topology and signal peptide predictions provided by Phobius you must download Phobius from https://phobius.sbc.su.se/data.html. As mentioned in the "use" section to utilize Phobius when running ProteoSeeker through the command-line the user should also provide the path to the Phobius directory in the parameter file or as a parameter through the corresponding option. The default path for the Phobiu installation in a Docker container from the proteoseeker Docker image is already set to the phobius directory of the shared directory as explained below, though the user should download and copy the Phobius installation files in that directory. In any other case, ProteoSeeker will run without performing topology and signal peptide predictions in its seek functionality.</p>
...
<p align="justify">The "phobius" directory in the bind mount or volume can be used to store the files of the Phobius installation. If done so, ProteoSeeker will detect and use Phobius when running in the Docker container. It is possible to store the Phobius installation in another directory and set ProteoSeeker to utilize by modifying the phobius path in the parameter file or by providing it as a parameter.</p>
