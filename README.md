...
<p align="justify">ProteoSeeker can be run as a Python module by importing the function "enzannmtg" and utilizing it in another Python script. The only requirements is for the latter script to be located at the same directory as "proteoseeker.py" and to be run in the conda environment "ps_env" (or any other user-defined suitable environment for running "proteoseeker.py"). The easiest way of utilizing ProteoSeeker as a module is to run it by providing as input a parameter file, similarly to running it through the command-line. In this case, providing the parameter file as an input means providing as an argument the path of the file in the corresponding parameter of ProteoSeeler's module. Each option of ProteoSeeker provided through the command-line is also available as a parameter when utilizing ProteoSeeker as a module. The order of the options of ProteoSeeker as a command-line tool is the same with the order of its parameters as a module. An example of utilizing ProteoSeeker as a module is provided based on the Python script "ps_mod.py". The latter script utilizes ProteoSeeker based on the parameter file "par_seek_tax_k_p.txt", which should be present in the same directory as "ps_mod.py" and "proteoseeker.py". As in the case of running ProteoSeeker through the command-line, the values provided for the options of ProteoSeeker in the parameter file overshadow the arguments provided for its parameters when utilized as a module.</p>
