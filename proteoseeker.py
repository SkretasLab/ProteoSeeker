...
def kraken_filtering(kraken_threshold, kraken_species_absab_dict, total_absab, absab_max, kraken_report_path, kraken_species_thr_path):
    first_kt = True
    ...
    for kt in kraken_threshold:
        ...
        if kt in [-1, -2]:
            ...
            for key in kraken_species_absab_dict.keys():
                if total_absab != 0:
                    ...
                    temp_prop = temp_absab / total_absab
                    if temp_prop != 0:
            ...
            if kraken_prop is not None:
                ...
                kraken_prop = kraken_prop * 0.01
                kraken_abs = absab_max * kraken_prop
        ...
        if kraken_prop is None:
            ...
            kraken_prop_temp = round(kraken_prop, 2)
        if kraken_abs is None:
            kraken_abs = "None"
            kraken_abs_temp = "None"
        else:
            kraken_abs_temp = round(kraken_abs, 2)
        abs_sum = 0
        perc_sum = 0
        ...
        with open(kraken_report_path) as report_lines:
            for line in report_lines:
                ...
                if clade_type == "S":
                    ...
                    total_absab += taxon_count
                    if taxon_count > absab_max:
                        absab_max = taxon_count
    # Kraken filtering thresholds are applied.s
    kraken_species_thr_dict = kraken_filtering(kraken_threshold, kraken_species_absab_dict, total_absab, absab_max, kraken_report_path, kraken_species_thr_path)
    ...
    if os.path.exists(output_final_contigs_formated):
        if bowtie_status:
            ...
            if bowtie_build_path:
                phrase_1 = "\"{}\" --threads {} \"{}\" \"{}\" &> \"{}\"".format(bowtie_build_path, thread_num, output_final_contigs_formated, output_final_contigs_formated, bowtie_build_stdoe_path)
                phrase_2 = "\"{}\" --version > \"{}\"".format(bowtie_build_path, bowtie_build_version_path)
            else:
                phrase_1 = "bowtie2-build --threads {} \"{}\" \"{}\" &> \"{}\"".format(thread_num, output_final_contigs_formated, output_final_contigs_formated, bowtie_build_stdoe_path)
