...
def help_message():
    ...
    help_notes_dict = {
        ...
        "-osi/--arg_only_seek_information": "True/False -Opt: False- Determines whether the results in the TXT and the EXCEL file will only contain information for the proteins identified through the seek mode (True) or also for any of the proteins taht belong to a bin (False). This is usefull in case information is only needed for the proteins found through the seek mode but for which also taxonomic information is needed, hence the taxonomic mode was also used",
        ...
        "-st/--score-type": "Str -Opt cut_ga- The scoring method used by HMMER. 'cut_ga': HMMER will use the profile's GA gathering cutoffs to set all thresholding. 'default': HMMER will use its default scoring method.",
        ...
        "-t/--threads": "Int -Opt: 4- The maximum number of threads to be used by any of the processes run by ProteoSeeker.",
...
def craete_phmm_db(family_code, family_group_name, enzyme_domains_folder, fam_nums_file_name, fam_pfam_file_name, pfam_domains_names, hmmer_env, hmmfetch_path, hmmpress_path, profiles_broad_path, input_log_file, output_log_file, ana_bin_path):
    ...
    if family_code is not None:
        ...
        if not os.path.exists(phmm_database_folder_name):
            ...
            new_file_bash.write("{}\n".format(phrase))
            if hmmer_env:
                phrase = "source {}/activate {}".format(ana_bin_path, hmmer_env)
                new_file_bash.write("{}\n".format(phrase))
            if hmmfetch_path:
...
def find_fam_names(family_code, family_group_name, create_nr_db_status, protein_db_path, fpr_db_gen_folder, fam_nums_file_name, name_thr, input_protein_names_status, input_protein_names, enzyme_names_dict, ana_bin_path):
...
def process_nr_db(fpr_db_folder, diamond_db_bash_name, diamond_env, diamond_path, fpr_db_fasta, fpr_db_name, diamond_makedb_version_path, diamond_makedb_stdoe_path, thread_num, input_log_file, output_log_file, ana_bin_path):
    ...
    if diamond_env:
        phrase = "source {}/activate {}".format(ana_bin_path, diamond_env)
        new_file_bash.write("{}\n".format(phrase))
    if diamond_path:
...
def create_nr_db(enzyme_names_dict, protein_db_path, diamond_db_bash_name, diamond_env, diamond_path, thread_num, input_log_file, output_log_file, ana_bin_path):
    ...
    if (not enz_names_analysis) and (not enz_names_phylo):
        ...
        pdfiltering.prfilter(protein_db_path, info_file_path, db_info_dict, log_file_path, thread_num)
    ...
    if enz_names_analysis:
        ...
        process_nr_db(fpr_db_folder, diamond_db_bash_name, diamond_env, diamond_path, fpr_db_fasta, fpr_db_name, diamond_makedb_version_path, diamond_makedb_stdoe_path, thread_num, input_log_file, output_log_file, ana_bin_path)
    if enz_names_phylo:
        ...
        process_nr_db(fpr_db_folder_phylo, diamond_db_bash_name, diamond_env, diamond_path, fpr_db_fasta_phylo, fpr_db_name_phylo, diamond_makedb_version_path, diamond_makedb_stdoe_path, thread_num, input_log_file, output_log_file, ana_bin_path)


def collect_sra(sra_env, sra_folder, prefetch_size, sra_run_folder, sra_file, fastq_folder, fastq_single_end, sra_code, input_log_file, output_log_file, prefetch_path, vdb_validate_path, fastq_dump_path, sra_bash_script, ana_bin_path):
    ...
    if os.path.exists(fastq_folder):
        ...
        new_file_bash.write("{}\n".format(phrase))
        if sra_env:
            phrase = "source {}/activate {}".format(ana_bin_path, sra_env)
            new_file_bash.write("{}\n".format(phrase))
        if prefetch_path:
...
def fastqc(fastqc_env, output_path_fastqc, file_paths, fastqc_path, clear, thread_num, input_log_file, output_log_file, fastqc_bash_script, fastqc_version_path, fastqc_stdoe_path, ana_bin_path):
    print("\nRunning FastQC...")
    if clear:
    ...
    new_file_bash.write("{}\n".format(phrase))
    if fastqc_env:
        phrase = "source {}/activate {}".format(ana_bin_path, fastqc_env)
        new_file_bash.write("{}\n".format(phrase))
    if fastqc_path:
...
def bbduk(bbduk_env, output_path_trimmed, file_paths, adapters_ap_path, paired_end, file_paths_p, bbduk_path, thread_num, bbduk_max_ram, output_path_summaries_errors, input_log_file, output_log_file, bbduk_bash_script, bbduk_version_path, bbduk_stdoe_path, ana_bin_path):
    ...
    if paired_end:
        for key in file_paths_p.keys():
            ...
            new_file_bash.write("{}\n".format(phrase))
            if bbduk_env:
                phrase = "source {}/activate {}".format(ana_bin_path, bbduk_env)
                new_file_bash.write("{}\n".format(phrase))
            if bbduk_path:
...
def megahit(megahit_env, output_path_megahit, tr_ex_file_paths, paired_end, tr_ex_file_paths_p, megahit_path, k_list, thread_num, input_log_file, output_log_file, megahit_bash_script, megahit_version_path, megahit_stdoe_path, ana_bin_path):
    print("\nRunning Megahit...")
    ...
    new_file_bash.write("{}\n".format(phrase))
    if megahit_env:
        phrase = "source {}/activate {}".format(ana_bin_path, megahit_env)
        new_file_bash.write("{}\n".format(phrase))
    new_file_bash.write("{}\n".format(phrase_1))
    new_file_bash.write("{}\n".format(phrase_2))
    if megahit_env:
...
def kraken(paired_end, tr_ex_file_paths_p, tr_ex_file_paths, output_path_kraken, kraken_db_path, kraken_results_path, kraken_report_path, kraken_threshold, kraken_species_path, kraken_species_thr_path, kraken_reads_path, kraken_taxname_path, ana_bin_path, kraken_env, kraken_path, kraken_bash_script, kraken_stde_path, kraken_version_path, kraken_status, kraken_memory_mapping, thread_num, input_log_file, output_log_file):
    print("\nRunning Kraken2...")
    ...
    if kraken_status:
        ...
        new_file_bash.write("{}\n".format(phrase))
        if kraken_env:
            phrase = "source {}/activate {}".format(ana_bin_path, kraken_env)
            new_file_bash.write("{}\n".format(phrase))
        if kraken_path:
        ...
        with open(kraken_report_path) as report_lines:
            for line in report_lines:
                ...
                if clade_type == "S":
                    ...
                    # clade_count = int(line_splited[1])
                    taxon_count = int(line_splited[2])
                    taxonid = int(line_splited[4])
                    kraken_species_dict[taxonid] = abu_perc
                    if taxon_count >= kraken_threshold:
                        kraken_species_thr_dict[taxonid] = abu_perc
    ...
    for key in kraken_species_dict.keys():
        abu_perc = kraken_species_dict[key]
        new_file_species.write("{}\t{}\n".format(key, abu_perc))
    new_file_species.close()
    abu_sum = 0
    new_file_species_thr = open(kraken_species_thr_path, "w")
    for key in kraken_species_thr_dict.keys():
        abu_perc = kraken_species_thr_dict[key]
        abu_sum += abu_perc
        new_file_species_thr.write("{}\t{}\n".format(key, abu_perc))
    new_file_species_thr.write("Toal\t{}\n".format(abu_sum))
...
def binning(binning_tool, metabinner_env, comebin_env, contigs, protein_input, output_path_bin, metabinner_bin_path, comebin_bin_path, fullpath_contnigs_formated, bin_bam_folder, enz_dir, output_path_conitgs, full_coverage_folder, fullpath_tr_fastq_files, full_bin_results_folder, full_coverage_profile_path, thread_num, input_log_file, output_log_file, metabinner_bash_script, comebin_bash_script, bin_ram_ammount, bin_num_contig_len, bin_num_kmer, bin_gen_coverage_stdoe_path, bin_gen_kmer_stdoe_path, bin_filter_tooshort_stdoe_path, binning_stdoe_path, comebin_batch_size, ana_bin_path):
    ...
    if binning_tool == 1:
        ...
        full_filtered_contigs_path = "{}/{}/contigs_formated_kmer_4_f{}.csv".format(enz_dir, output_path_conitgs, bin_num_contig_len)
        run_metabinner_path = "{}/run_metabinner.sh".format(metabinner_bin_path)
        contnigs_formated_splited = fullpath_contnigs_formated.split(".")
        ...
        phrase_1 = "{} -a {} -o {} -b {} -t {} -m {} -l {} {} &> {}".format(gen_coverage_file_path, fullpath_contnigs_formated, full_coverage_folder, bin_bam_folder, thread_num, bin_ram_ammount, bin_num_contig_len, fullpath_tr_fastq_files, bin_gen_coverage_stdoe_path)
        # Create kmers.
        phrase_2 = "{} {} {} {} &> {}".format(gen_kmer_path, fullpath_contnigs_formated, bin_num_contig_len, bin_num_kmer, bin_gen_kmer_stdoe_path)
        # Filter contigs based on their lengths.
        phrase_3 = "{} {} {} &> {}".format(filter_tooshort_path, fullpath_contnigs_formated, bin_num_contig_len, bin_filter_tooshort_stdoe_path)
        if place_python:
        ...
        new_file_bash.write("{}\n".format(phrase))
        if metabinner_env:
            phrase = "source {}/activate {}".format(ana_bin_path, metabinner_env)
            new_file_bash.write("{}\n".format(phrase))
        new_file_bash.write("{}\n".format(phrase_1))
        new_file_bash.write("{}\n".format(phrase_2))
        new_file_bash.write("{}\n".format(phrase_3))
        ...
        # Get the coverage profiles.
        phrase_2 = "{} -a {} -o {} -b {} -t {} -m {} -l {} {} &> {}".format(gen_coverage_file_path, fullpath_contnigs_formated, full_coverage_folder, bin_bam_folder, thread_num, bin_ram_ammount, bin_num_contig_len, fullpath_tr_fastq_files, bin_gen_coverage_stdoe_path)
        # Filter contigs based on their lengths.
        phrase_3 = "{} {} {} &> {}".format(filter_tooshort_path, fullpath_contnigs_formated, bin_num_contig_len, bin_filter_tooshort_stdoe_path)
        # Bin the contigs.
        ...
        new_file_bash.write("{}\n".format(phrase))
        if comebin_env:
            phrase = "source {}/activate {}".format(ana_bin_path, comebin_env)
            new_file_bash.write("{}\n".format(phrase))
        new_file_bash.write("{}\n".format(phrase_1))
        new_file_bash.write("{}\n".format(phrase_2))
        new_file_bash.write("{}\n".format(phrase_3))
...
def gene_prediction(genepred_env, output_path_genepred, output_path_genepred_base, output_path_genepred_faa, output_fgs_protein_formatted_1, fullpath_contnigs_formated, fraggenescanrs_path, thread_num, input_log_file, output_log_file, gene_bash_script, fraggenescanrs_version_path, fraggenescanrs_stdoe_path, ana_bin_path):
    ...
    if fraggenescanrs_path:
        phrase_1 = "{} -w 1 -o {} -s {} -p {} --training-file complete &> {}".format(fraggenescanrs_path, output_path_genepred_base, fullpath_contnigs_formated, thread_num, fraggenescanrs_stdoe_path)
        phrase_2 = "{} --version > {}".format(fraggenescanrs_path, fraggenescanrs_version_path)
    else:
        phrase_1 = "FragGeneScanRs -w 1 -o {} -s {} -p {} --training-file complete &> {}".format(output_path_genepred_base, fullpath_contnigs_formated, thread_num, fraggenescanrs_stdoe_path)
    ...
    new_file_bash.write("{}\n".format(phrase))
    if genepred_env:
        phrase = "source {}/activate {}".format(ana_bin_path, genepred_env)
        new_file_bash.write("{}\n".format(phrase))
    new_file_bash.write("{}\n".format(phrase_1))
    new_file_bash.write("{}\n".format(phrase_2))
    if genepred_env:
...
def cd_hit(cdhit_env, output_path_cdhit, prs_source, output_fgs_protein_formatted_1, output_fgs_protein_formatted_2, cd_hit_results_path, cd_hit_results_fasta_path, cd_hit_t, cd_hit_path, cd_hit_mem, thread_num, input_log_file, output_log_file, cdhit_bash_script, cd_hit_version_path, cd_hit_stdoe_path, ana_bin_path):
    print("\nRunning CD-HIT...")
    ...
    if os.path.exists(output_proteins_formatted):
        ...
        new_file_bash.write("{}\n".format(phrase))
        if cdhit_env:
            phrase = "source {}/activate {}".format(ana_bin_path, cdhit_env)
            new_file_bash.write("{}\n".format(phrase))
        new_file_bash.write("{}\n".format(phrase_1))
        new_file_bash.write("{}\n".format(phrase_2))
        if cdhit_env:
...
def analyze_no_enzs(diamond_env, output_path_hmmer, output_path_blastp, prs_with_enz_domains, blastp_results_no_doms_nr_file, blastp_info_no_doms_nr_file, blastp_no_doms_below_threshold, blastp_info_no_doms_below_threshold, cd_hit_results_path, file_prs_seq_no_enzs_name, fpr_db_fasta, fpr_db_name, e_value_nodom_thr, thread_num, input_log_file, output_log_file, diamond_path, blastp_nodoms_script, diamond_blastp_nr1_stdoe_path, ana_bin_path):
...
def blastp(diamond_env, output_path_blastp, blastp_results_swissprot_file, blastp_doms_nr_file, blastp_info_swissprot_file, blastp_doms_info_nr_file, file_prs_seq_enz_domains_name, proteins_combined_file, swissprot_path, fpr_db_fasta, fpr_db_name, thread_num, input_log_file, output_log_file, diamond_path, blast_swissprot_bash_script, blast_nr_bash_script, phylo_analysis, ana_bin_path, diamond_blastp_swissprot_stdoe_path, diamond_blastp_nr2_stdoe_path):
    ...
    if os.path.exists(file_prs_seq_enz_domains_name):
        ...
        if os.path.exists(fpr_db_fasta):
            if diamond_path:
                ...
                phrase_1 = "diamond blastp {} --threads {} --db {} --out {} --query {} --outfmt 6 &> {}".format(diamond_path, thread_num, fpr_db_name, blastp_doms_nr_file, file_prs_seq_enz_domains_name, diamond_blastp_nr2_stdoe_path)
                phrase_2 = ""
            # Create the Bash script.
            # Four cases: 1: Conda enviroment and path needed for the script. 2: Conda enviroment and no path needed for the script. 3: No conda enviroment and path needed for the script. 4: No conda enviroment and no path needed for the script.
...
def bin_phylo(fpr_db_fasta, blastp_doms_info_nr_file, dict_contigs_bins, binning_results_path, output_path_genepred, output_path_genepred_faa, binphylo_path_prstax, binphylo_path_binstax, binphylo_path_binstax_max, binphylo_path_prsids, binphylo_path_binstax_names, binphylo_path_binstax_max_names, binphylo_freq_taxids_path, binphylo_maxfreq_taxids_path, cd_hit_results_fasta_path, taxonkit_env, taxonkit_path, tax_freq_bash_script, tax_maxfreq_bash_script, taxoknit_freq_version_path, taxoknit_freq_stdoe_path, taxoknit_maxfreq_version_path, taxoknit_maxfreq_stdoe_path, thread_num, ana_bin_path, freq_taxids_path, maxfreq_taxids_path, taxonkit_freq_line_bash_script, taxonkit_maxfreq_line_bash_script, taxonkit_freq_line_version_path, taxonkit_freq_line_stdoe_path, taxonkit_maxfreq_line_version_path, taxonkit_maxfreq_line_stdoe_path, freq_lineage_path, maxfreq_lineage_path, freq_lineage_form_path, maxfreq_lineage_form_path, csvtk_freq_version_path, csvtk_freq_stdoe_path, csvtk_maxfreq_version_path, csvtk_maxfreq_stdoe_path, family_to_profile_phylo_dict, hmmer_enz_domains_all_proteins_phylo, family_profile_path, input_log_file, output_log_file):
        ...
        with open(fpr_db_fasta) as pnr_lines:
            for line in pnr_lines:
                ...
                if line[0] == ">":
                    ...
                    result_tax = re.findall(pattern_tax, line)
                    if result_tax:
                        nr_tax = []
                        # Count the species related to the protein sequence.
                        nr_tax_dict = {}
                        for taxum in result_tax:
    ...
    for key_bin in dict_bins_prs_sorted.keys():
        for pr_ac in dict_bins_prs_sorted[key_bin]:
            if pr_ac not in bin_pr_accs:
                bin_pr_accs.append(pr_ac)
    ...
    if os.path.exists(binphylo_path_binstax_names):
        ...
        new_file_bash.write("{}\n".format(phrase))
        if taxonkit_env:
            phrase = "source {}/activate {}".format(ana_bin_path, taxonkit_env)
            new_file_bash.write("{}\n".format(phrase))
        new_file_bash.write("{}\n".format(phrase_1))
        new_file_bash.write("{}\n".format(phrase_2))
        if taxonkit_env:
...
def phobius(phobius_env, proteins_combined_file, output_path_topology, phobius_input_file_name, phobius_output_file_name, topology_info_path, phobius_path, input_log_file, output_log_file, phobius_bash_script, phobius_version_path, phobius_stde_path, ana_bin_path):
    print("\nRunning Phobius...")
    ...
    if os.path.exists(proteins_combined_file):
        ...
        new_file_bash.write("{}\n".format(phrase))
        if phobius_env:
            phrase = "source {}/activate {}".format(ana_bin_path, phobius_env)
            new_file_bash.write("{}\n".format(phrase))
        new_file_bash.write("{}\n".format(phrase_1))
        new_file_bash.write("{}\n".format(phrase_2))
        if phrase_3:
...
def bowtie(output_path_bowtie, paired_end, tr_ex_file_paths, tr_ex_file_paths_p, ana_bin_path, bowtie_build_path, bowtie_build_version_path, bowtie_build_stdoe_path, bowtie_env, bowtie_bash_script, bowtie_path, bowtie_version_path, bowtie_stdoe_path, bowtie_single_unaligned_path, bowtie_paired_unaligned_con_path, bowtie_stats_path, mapped_reads_path, thread_num, output_final_contigs_formated, bowtie_status, input_log_file, output_log_file):
    print("\nRunning bowtie2...")
    ...
    if os.path.exists(output_final_contigs_formated):
        if bowtie_status:
            ...
            if bowtie_build_path:
                phrase_1 = "{} {} {} &> {}".format(bowtie_build_path, output_final_contigs_formated, output_final_contigs_formated, bowtie_build_stdoe_path)
                phrase_2 = "{} --version > {}".format(bowtie_build_path, bowtie_build_version_path)
            else:
                phrase_1 = "bowtie2-build {} {} &> {}".format(output_final_contigs_formated, output_final_contigs_formated, bowtie_build_stdoe_path)
            ...
            if paired_end:
                ...
                for key_file in range(0, len(tr_ex_file_paths_keys)):
                    ...
                    if key_file + 1 == len(tr_ex_file_paths_keys):
                        ...
                        paired_two_phrase = "{}{},".format(paired_two_phrase, file_path_2)
                if bowtie_path:
                    phrase_3 = "{} -x {} -1 {} -2 {} --very-sensitive --un {} --un-conc {} --met-file {} --omit-sec-seq -p {} -S {} &> {}".format(bowtie_path, output_final_contigs_formated, paired_one_phrase, paired_two_phrase, bowtie_single_unaligned_path, bowtie_paired_unaligned_con_path, bowtie_stats_path, thread_num, mapped_reads_path, bowtie_stdoe_path)
                    phrase_4 = "{} --version > {}".format(bowtie_path, bowtie_version_path)
                else:
                    phrase_3 = "bowtie2 -x {} -1 {} -2 {} --very-sensitive --un {} --un-conc {} --met-file {} --omit-sec-seq -p {} -S {} &> {}".format(output_final_contigs_formated, paired_one_phrase, paired_two_phrase, bowtie_single_unaligned_path, bowtie_paired_unaligned_con_path, bowtie_stats_path, thread_num, mapped_reads_path, bowtie_stdoe_path)
                ...
                for i in range(0, len(tr_ex_file_paths)):
                    ...
                    if i + 1 == len(tr_ex_file_paths):
                        ...
                        single_phrase = "{}{},".format(single_phrase, file_path)
                if bowtie_path:
                    phrase_3 = "{} -x {} -U {} --very-sensitive --un {} --un-conc {} --met-file {} --omit-sec-seq -p {} -S {} &> {}".format(bowtie_path, output_final_contigs_formated, single_phrase, bowtie_single_unaligned_path, bowtie_paired_unaligned_con_path, bowtie_stats_path, thread_num, mapped_reads_path, bowtie_stdoe_path)
                    phrase_4 = "{} --version > {}".format(bowtie_path, bowtie_version_path)
                else:
                    phrase_3 = "bowtie2 -x {} -U {} --very-sensitive --un {} --un-conc {} --met-file {} --omit-sec-seq -p {} -S {} &> {}".format(bowtie_path, output_final_contigs_formated, single_phrase, bowtie_single_unaligned_path, bowtie_paired_unaligned_con_path, bowtie_stats_path, thread_num, mapped_reads_path, bowtie_stdoe_path)
                    phrase_4 = "bowtie2 --version > {}".format(bowtie_version_path)
            # Create the Bash script.
            # Four cases: 1: Conda enviroment and path needed for the script. 2: Conda enviroment and no path needed for the script. 3: No conda enviroment and path needed for the script. 4: No conda enviroment and no path needed for the script.
            ...
            new_file_bash.write("{}\n".format(phrase))
            if bowtie_env:
                phrase = "source {}/activate {}".format(ana_bin_path, bowtie_env)
                new_file_bash.write("{}\n".format(phrase))
            new_file_bash.write("{}\n".format(phrase_1))
            new_file_bash.write("{}\n".format(phrase_2))
            new_file_bash.write("{}\n".format(phrase_3))
    ...
    if kraken_mode:
        ...
        for key_contig in contig_to_species_max_single.keys():
            ...
            for species_id in contig_to_species_max_single[key_contig].keys():
                ...
                for protein_id in dict_prs_bins.keys():
                    pr_bin_id = dict_prs_bins[protein_id]
                    if bin_id == pr_bin_id:
                        bin_group_tax_dict[bin_id]["protein_ids_k"].append(protein_id)
    ...
    if dict_bins_prs_sorted:
        for key_bin in dict_bins_prs_sorted.keys():
            ...
            for pr_item in binned_prs:
                if pr_item not in binned_protein_ids:
                    binned_protein_ids.append(pr_item)
    # Sequences
...
def enzannmtg(input_folder=None, sra_code=False, contigs=False, protein_input=False, adapters_path="adapters.fa", protein_db_path="", kraken_db_path="", profiles_path="", profiles_phylo_path="", profiles_broad_path="", swissprot_path="", motifs_path="motifs.txt", output_path="", input_command=None, options_file_path="", family_code=None, family_code_phylo=None, db_name="", db_name_phylo="", input_protein_names_status=False, input_protein_names=None, input_protein_names_phylo_status=False, input_protein_names_phylo=None, name_thr=0.5, analysis_mode=3, paired_end=True, compressed=True, create_nr_db_status=False, prefetch_size=20, adapters_status="pre", only_seek_info=False, skip_fastqc=False, bbduk_max_ram=4, clear_space=False, k_list=None, kraken_mode=True, kraken_threshold=100, kraken_memory_mapping=True, binning_tool=1, bin_ram_ammount=4, bin_num_contig_len=500, bin_num_kmer=4, comebin_batch_size=256, cd_hit_t=0.99, cd_hit_mem=4000, prs_source=1, genetic_code=11, val_type="--cut_ga ", second_dom_search=True, e_value_nodom_thr=1e-70, add_type=[], add_info=[], thread_num=4, after_trimming=False, after_alignment=False, after_gene_pred=False, after_binning=False, after_db=False, after_tm=False, after_ap=False, up_to_databases=False, up_to_trimming_com=False, up_to_trimming_uncom=False, up_to_alignment=False, sra_env="ps_sra_tools", fastqc_env="ps_fastqc", bbduk_env="ps_bbtools", megahit_env="ps_megahit", kraken_env="ps_kraken", metabinner_env="ps_metabinner", comebin_env="ps_comebin", cdhit_env="ps_cd_hit", genepred_env="", hmmer_env="ps_hmmer", diamond_env="ps_diamond", taxonkit_env="ps_taxonkit", phobius_env="ps_phobius", bowtie_env="ps_bowtie", ana_bin_path="", prefetch_path="", vdb_validate_path="", fastq_dump_path="", fastqc_path="", gzip_path="", cat_path="", bbduk_path="", megahit_path="", kraken_path="", metabinner_bin_path="", comebin_bin_path="", fraggenescanrs_path="", hmmscan_path="", hmmpress_path="", hmmfetch_path="", diamond_path="", cd_hit_path="", taxonkit_path="", phobius_path="", bowtie_build_path="", bowtie_path=""):
    # Start time
            ...
            if line:
                ...
                if "=" in line:
                    ...
                    if option_value and option_value != "\"\"":
                        ...
                        # General options
                        # Pipeline
                        if option_type == "analysis_mode":
                            analysis_mode = int(option_value)
                        ...
                        if option_type == "only_seek_info":
                            only_seek_info = option_value
                            arg_str = "-only_seek_info"
                            only_seek_info = check_i_value(only_seek_info, arg_str)
                        # FastQC
                        ...
                        if option_type == "kraken_threshold":
                            kraken_threshold = int(option_value)
    ...
    if (not ana_bin_path) or (ana_bin_path == "") or (not os.path.exists(ana_bin_path)):
        print("\nThe path to the anaconda bin folder does not exist. Exiting.")
        exit()
    ...
    enz_dir = os.getcwd()
    # Binning.
    fullpath_contnigs_formated = "{}/{}/{}".format(enz_dir, output_path_conitgs, output_final_contigs_formated_name)
    fullpath_tr_fastq_files = "{}/{}/*fastq".format(enz_dir, output_path_trimmed)
    full_binning_folder = "{}/{}".format(enz_dir, output_path_bin)
    full_bin_results_folder = "{}/{}/bins".format(enz_dir, output_path_bin)
    ...
    kraken_species_thr_path = "{}/kraken_species_thr.tsv".format(output_path_kraken)
    ...
    if sra_code:
        paired_end, input_folder = collect_sra(sra_env, sra_folder, prefetch_size, sra_run_folder, sra_file, fastq_folder, fastq_single_end, sra_code, input_log_file, output_log_file, prefetch_path, vdb_validate_path, fastq_dump_path, sra_bash_script, ana_bin_path)
        compressed = True
        contigs = False
    ...
    profiles_path, analysis_fam_names, family_to_profile_seek_dict = craete_phmm_db(family_code, family_group_name, enzyme_domains_folder, fam_nums_file_name, fam_pfam_file_name, pfam_domains_names, hmmer_env, hmmfetch_path, hmmpress_path, profiles_broad_path, input_log_file, output_log_file, ana_bin_path)
    fpr_db_fasta, fpr_db_name, fpr_db_folder, enzyme_names_dict = find_fam_names(family_code, family_group_name, create_nr_db_status, protein_db_path, fpr_db_gen_folder, fam_nums_file_name, name_thr, input_protein_names_status, input_protein_names, enzyme_names_dict, ana_bin_path)
    # The phylo pHMM database.
    # Determine the phylo protein names.
    profiles_phylo_path, analysis_fam_names_phylo, family_to_profile_phylo_dict = craete_phmm_db(family_code_phylo, family_group_name_phylo, enzyme_domains_folder, fam_nums_file_name, fam_pfam_file_name, pfam_domains_names, hmmer_env, hmmfetch_path, hmmpress_path, profiles_broad_path, input_log_file, output_log_file, ana_bin_path)
    ...
    create_nr_db(enzyme_names_dict, protein_db_path, diamond_db_bash_name, diamond_env, diamond_path, thread_num, input_log_file, output_log_file, ana_bin_path)    
    # Time for databases
    label = "Process of creating the pHMM and fnr databases (including phylo):"
    end_time_analysis(label, start_time_dbs, output_log_file)

    # If chosen, the analysis stops here.
    if up_to_databases:
        label = "Databases creation:"
        end_time_analysis(label, start_time, output_log_file)
        exit()
    ...
    if (not contigs) and (not after_alignment) and (not after_gene_pred) and (not after_binning) and (not after_db) and (not protein_input) and (not after_tm) and (not after_ap):
        if not after_trimming:
            ...
            clear = True
            fastqc(fastqc_env, output_path_fastqc, file_paths, fastqc_path, clear, thread_num, input_log_file, output_log_file, fastqc_1_bash_script, fastqc_version_bt_path, fastqc_stdoe_bt_path, ana_bin_path)
            label = "Process of initial fastqc:"
            end_time_analysis(label, start_time_fastqc_initial, output_log_file)
            ...
            bbduk(bbduk_env, output_path_trimmed, file_paths, adapters_ap_path, paired_end, file_paths_p, bbduk_path, thread_num, bbduk_max_ram, output_path_summaries_errors, input_log_file, output_log_file, bbduk_bash_script, bbduk_version_path, bbduk_stdoe_path, ana_bin_path)
            label = "Process of trimming:"
            end_time_analysis(label, start_time_trimming, output_log_file)
            ...
            if not skip_fastqc:
                ...
                clear = False
                fastqc(fastqc_env, output_path_fastqc, ca_file_paths, fastqc_path, clear, thread_num, input_log_file, output_log_file, fastqc_2_bash_script, fastqc_version_at_path, fastqc_stdoe_at_path, ana_bin_path)
                label = "Process of final fastqc:"
                end_time_analysis(label, start_time_fastqc_final, output_log_file)
        ...
        megahit(megahit_env, output_path_megahit, tr_ex_file_paths, paired_end, tr_ex_file_paths_p, megahit_path, k_list, thread_num, input_log_file, output_log_file, megahit_bash_script, megahit_version_path, megahit_stdoe_path, ana_bin_path)
        label = "Process of read assembly:"
        end_time_analysis(label, start_time_assembly, output_log_file)
    ...
    if (not after_db) and (not protein_input) and (not after_binning) and (not after_tm) and (not after_ap):
        if not after_gene_pred:
            ...
            end_time_analysis(label, start_time_genepred, output_log_file)
        ...
        if (not only_phylo_mode):
            ...
            end_time_analysis(label, start_time_geneann, output_log_file)

        # Protein redundancy.
        start_time_cd_hit = time.time()
        cd_hit(cdhit_env, output_path_cdhit, prs_source, output_fgs_protein_formatted_1, output_fgs_protein_formatted_2, cd_hit_results_path, cd_hit_results_fasta_path, cd_hit_t, cd_hit_path, cd_hit_mem, thread_num, input_log_file, output_log_file, cdhit_bash_script, cd_hit_version_path, cd_hit_stdoe_path, ana_bin_path)
        label = "Process of cd-hit:"
        end_time_analysis(label, start_time_cd_hit, output_log_file)
    ...
    if (not after_db) and (not protein_input) and (not after_tm) and (not after_ap):
        if not after_binning:
            ...
            if (not only_seek_mode) and kraken_mode:
                ...
                kraken_status = True
                kraken_species_dict, kraken_species_thr_dict, read_to_species_dict, taxid_to_species_dict = kraken(paired_end, tr_ex_file_paths_p, tr_ex_file_paths, output_path_kraken, kraken_db_path, kraken_results_path, kraken_report_path, kraken_threshold, kraken_species_path, kraken_species_thr_path, kraken_reads_path, kraken_taxname_path, ana_bin_path, kraken_env, kraken_path, kraken_bash_script, kraken_stde_path, kraken_version_path, kraken_status, kraken_memory_mapping, thread_num, input_log_file, output_log_file)
                label = "Process of kraken2:"
                end_time_analysis(label, start_time_kraken, output_log_file)
            ...
            if (not only_seek_mode) and (not kraken_mode):
                ...
                binning(binning_tool, metabinner_env, comebin_env, contigs, protein_input, output_path_bin, metabinner_bin_path, comebin_bin_path, fullpath_contnigs_formated, bin_bam_folder, enz_dir, output_path_conitgs, full_coverage_folder, fullpath_tr_fastq_files, full_bin_results_folder, full_coverage_profile_path, thread_num, input_log_file, output_log_file, metabinner_bash_script, comebin_bash_script, bin_ram_ammount, bin_num_contig_len, bin_num_kmer, bin_gen_coverage_stdoe_path, bin_gen_kmer_stdoe_path, bin_filter_tooshort_stdoe_path, binning_stdoe_path, comebin_batch_size, ana_bin_path)
                label = "Process of binning:"
                end_time_analysis(label, start_time_binning, output_log_file)
        ...
        if not only_seek_mode:
            ...
            bowtie_status = True
            reads_to_contigs_dict, contigs_to_reads_dict = bowtie(output_path_bowtie, paired_end, tr_ex_file_paths, tr_ex_file_paths_p, ana_bin_path, bowtie_build_path, bowtie_build_version_path, bowtie_build_stdoe_path, bowtie_env, bowtie_bash_script, bowtie_path, bowtie_version_path, bowtie_stdoe_path, bowtie_single_unaligned_path, bowtie_paired_unaligned_con_path, bowtie_stats_path, mapped_reads_path, thread_num, output_final_contigs_formated, bowtie_status, input_log_file, output_log_file)
            label = "Process of read mapping:"
            end_time_analysis(label, start_time_bowtie, output_log_file)
    ...
    if (not after_tm) and (not after_ap):
        if not after_db:
            if (analysis_mode == 1 or analysis_mode == 3):
                if not only_phylo_mode:
                    ...
                    end_time_analysis(label, start_time_hmmer_spec, output_log_file)
                ...
                if profiles_phylo_path:
                    ...
                    label = "Process of specified phylo HMMER:"
                    end_time_analysis(label, start_time_hmmer_spec_phylo, output_log_file)

            if (analysis_mode == 2 or analysis_mode == 3) and (not only_phylo_mode):
                ...
                label = "Process of BLASTP against fnr for protein without domains.:"
                end_time_analysis(label, start_time_nr_no_doms, output_log_file)
            ...
            if (not only_phylo_mode):
                ...
                end_time_analysis(label, start_time_nr_swiss_doms, output_log_file)
        ...
        if not after_tm:
            if not only_seek_mode:
                if not kraken_mode:
                    ...
                    end_time_analysis(label, start_time_metacome, output_log_file)
                ...
                if kraken_mode:
                    ...
                    end_time_analysis(label, start_time_kraken_binning, output_log_file)
    ...
    if not only_phylo_mode:
        if not after_ap:
            if not after_tm:
                ...
                end_time_analysis(label, start_time_hmmer_broad, output_log_file)
                
                # Topology predictions
                start_time_tm = time.time()
                dict_top = phobius(phobius_env, proteins_combined_file, output_path_topology, phobius_input_file_name, phobius_output_file_name, topology_info_path, phobius_path, input_log_file, output_log_file, phobius_bash_script, phobius_version_path, phobius_stde_path, ana_bin_path)
                label = "Process of topology prediction:"
                end_time_analysis(label, start_time_tm, output_log_file)
            ...
            end_time_analysis(label, start_time_motifs, output_log_file)
            ...
            end_time_analysis(label, start_time_swiss_fam_pred, output_log_file)
    ...
    end_time_analysis(label, start_time_info_collection, output_log_file)
    ...
    end_time_analysis(label, start_time_results, output_log_file)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        for i in range(1, len(sys.argv), 2):
            if sys.argv[i] == "-i" or sys.argv[i] == "--input":
            ...
            elif sys.argv[i] == "-am" or sys.argv[i] == "--analysis-mode":
                arg_analysis_mode = int(sys.argv[i+1])
            ...
            elif sys.argv[i] == "-osi" or sys.argv[i] == "--only-seek-information":
                arg_only_seek_info = sys.argv[i+1]
                arg_str = "-osi"
                arg_only_seek_info = check_i_value(arg_only_seek_info, arg_str)
                ...
                arg_kraken_threshold = int(sys.argv[i+1])
                ...
                if arg_bowtie_env in ["None", "none"]:
                    arg_bowtie_env = ""
            elif sys.argv[i] == "-abp" or sys.argv[i] == "--anaconda-bin-path":
                arg_ana_bin_path = sys.argv[i+1]
    ...
    enzannmtg(arg_input_folder, arg_sra_code, arg_contigs, arg_protein_input, arg_adapters_path, arg_protein_db_path, arg_kraken_db_path, arg_profiles_path, arg_profiles_phylo_path, arg_profiles_broad_path, arg_swissprot_path, arg_motifs_path, arg_output_path, arg_input_command, arg_options_file_path, arg_family_code, arg_family_code_phylo, arg_db_name, arg_db_name_phylo, arg_input_protein_names_status, arg_input_protein_names, arg_input_protein_names_phylo_status, arg_input_protein_names_phylo, arg_name_thr, arg_analysis_mode, arg_paired_end, arg_compressed, arg_create_nr_db_status, arg_prefetch_size, arg_adapters_status, arg_only_seek_info, arg_skip_fastqc, arg_bbduk_max_ram, arg_clear_space, arg_k_list, arg_kraken_mode, arg_kraken_threshold, arg_kraken_memory_mapping, arg_binning_tool, arg_bin_ram_ammount, arg_bin_num_contig_len, arg_bin_num_kmer, arg_comebin_batch_size, arg_cd_hit_t, arg_cd_hit_mem, arg_prs_source, arg_genetic_code, arg_val_type, arg_second_dom_search, arg_e_value_nodom_thr, arg_add_type, arg_add_info, arg_thread_num, arg_after_trimming, arg_after_alignment, arg_after_gene_pred, arg_after_binning, arg_after_db, arg_after_tm, arg_after_ap, arg_up_to_databases, arg_up_to_trimming_com, arg_up_to_trimming_uncom, arg_up_to_alignment, arg_sra_env, arg_fastqc_env, arg_bbduk_env, arg_megahit_env, arg_kraken_env, arg_metabinner_env, arg_comebin_env, arg_cdhit_env, arg_genepred_env, arg_hmmer_env, arg_diamond_env, arg_taxonkit_env, arg_phobius_env, arg_bowtie_env, arg_ana_bin_path, arg_prefetch_path, arg_vdb_validate_path, arg_fastq_dump_path, arg_fastqc_path, arg_gzip_path, arg_cat_path, arg_bbduk_path, arg_megahit_path, arg_kraken_path, arg_metabinner_bin_path, arg_comebin_bin_path, arg_fraggenescanrs_path, arg_hmmscan_path, arg_hmmpress_path, arg_hmmfetch_path, arg_diamond_path, arg_cd_hit_path, arg_taxonkit_path, arg_phobius_path, arg_bowtie_build_path, arg_bowtie_path)
