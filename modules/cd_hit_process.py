...
def cd_hit(cdhit_env, output_path_cdhit, prs_source, output_fgs_protein_formatted_1, output_fgs_protein_formatted_2, cd_hit_results_path, cd_hit_results_fasta_path, cd_hit_t, cd_hit_path, cd_hit_mem, thread_num, input_log_file, output_log_file, cdhit_bash_script, cd_hit_version_path, cd_hit_stdoe_path, conda_sh_path, cd_hit_par):
    print("\nRunning CD-HIT...")
    ...
    if cd_hit_par is None:
        cd_hit_par = " -M {} -T {} -g 1 ".format(cd_hit_mem, thread_num)
    else:
        if cd_hit_par[0] != " ":
            cd_hit_par = " {}".format(cd_hit_par)
        if cd_hit_par[-1] != " ":
            cd_hit_par = "{} ".format(cd_hit_par)
    ...
    if os.path.exists(output_proteins_formatted):
        if cd_hit_path:
            phrase_1 = "\"{}\" -i \"{}\" -c {} -o \"{}\"{}&> \"{}\"".format(cd_hit_path, output_proteins_formatted, cd_hit_t, cd_hit_results_path, cd_hit_par, cd_hit_stdoe_path)
            phrase_2 = "{} -h > {}".format(cd_hit_path, cd_hit_version_path)
        else:
            phrase_1 = "cd-hit -i \"{}\" -c {} -o \"{}\"{}&> \"{}\"".format(output_proteins_formatted, cd_hit_t, cd_hit_results_path, cd_hit_par, cd_hit_stdoe_path)
