...
def megahit(megahit_env, output_path_megahit, tr_ex_file_paths, paired_end, tr_ex_file_paths_p, megahit_path, k_list, thread_num, input_log_file, output_log_file, megahit_bash_script, megahit_version_path, megahit_stdoe_path, conda_sh_path, megahit_par):
    print("\nRunning Megahit...")
    ...
    # Tool parameters
    if megahit_par is None:
        megahit_par = " -t {} --presets meta-large ".format(thread_num)
        if k_list is not None:
            megahit_par = "{}--k-list=\"{}\"".format(megahit_par, k_list)
    else:
        if megahit_par[0] != " ":
            megahit_par = " {}".format(megahit_par)
        if megahit_par[-1] != " ":
            megahit_par = "{} ".format(megahit_par)
    ...
    if paired_end:
        if megahit_path:
            phrase_1 = "\"{}\"{}".format(megahit_path, megahit_par)
            phrase_2 = "\"{}\" --version > \"{}\"".format(megahit_path, megahit_version_path)
        else:
            phrase_1 = "megahit{}".format(megahit_par)
        ...
        phrase_1 = "{} -o \"{}/megahit_contigs\" &> \"{}\"".format(phrase_1, output_path_megahit, megahit_stdoe_path)
