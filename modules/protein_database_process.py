...
def process_pr_db(fpd_folder, diamond_db_bash_name, diamond_env, diamond_path, fpd_fasta, fpd_name, diamond_makedb_version_path, diamond_makedb_stdoe_path, thread_num, input_log_file, output_log_file, conda_sh_path):
    ...
    diamond_db_bash_script = "{}/{}".format(fpd_folder, diamond_db_bash_name)
    new_file_bash = open(diamond_db_bash_script, "w")
    phrase = "#!/bin/bash"
    new_file_bash.write("{}\n".format(phrase))
    phrase_1 = "cat \"{}/\"*.fasta > \"{}\"".format(fpd_folder, fpd_fasta)
    if diamond_env:
        ...
        new_file_bash.write("{}\n".format(phrase_a))
    if diamond_path:
        phrase_2 = "\"{}\" makedb --threads {} --in \"{}\" --db \"{}\" &> \"{}\"".format(diamond_path, thread_num, fpd_fasta, fpd_name, diamond_makedb_stdoe_path)
        phrase_3 = 'echo "No version available for diamond makedb." > {}'.format(diamond_makedb_version_path)
    else:
        phrase_2 = "diamond makedb --threads {} --in \"{}\" --db \"{}\" &> \"{}\"".format(thread_num, fpd_fasta, fpd_name, diamond_makedb_stdoe_path)
        phrase_3 = 'echo "No version available for diamond makedb." > \"{}\"'.format(diamond_makedb_version_path)
    new_file_bash.write("{}\n".format(phrase_1))
    new_file_bash.write("{}\n".format(phrase_2))
    new_file_bash.write("{}\n".format(phrase_3))
    ...
    pd_folder_files = os.listdir(fpd_folder)
    for pd_file in pd_folder_files:
        pd_file_path = "{}/{}".format(fpd_folder, pd_file)
...
def create_pr_db(pr_names_dict, protein_db_path, diamond_db_bash_name, diamond_env, diamond_path, thread_num, pdf_threads, input_log_file, output_log_file, conda_sh_path):
    pr_names_seek = pr_names_dict[0][0]
    pr_names_taxonomy = pr_names_dict[1][0]

    # Duplicate names are removed.
    if (not pr_names_seek) and (not pr_names_taxonomy):
        print("\nNo protein names were found. The protein database will not be filtered.")
    else:
        fpd_fasta_seek = pr_names_dict[0][1]
        fpb_fasta_taxonomy = pr_names_dict[1][1]
        fpd_folder_seek = pr_names_dict[0][2]
        fpr_db_folder_taxonomy = pr_names_dict[1][2]
        fpd_name_seek = pr_names_dict[0][3]
        fpd_name_taxonomy = pr_names_dict[1][3]
        fpd_fasta_prefix = pr_names_dict[0][4]
        fpd_fasta_prefix_taxonomy  = pr_names_dict[1][4]
        # Create the folder for the database.
        if pr_names_seek:
            os.mkdir(fpd_folder_seek)
        if pr_names_taxonomy:
            os.mkdir(fpr_db_folder_taxonomy)
        print("\nNames collected to create the seek pnr: {}".format(pr_names_seek))
        print("Names collected to create the phylo pnr: {}".format(pr_names_taxonomy))
        if pr_names_seek:
            fpr_seeknames_path = "{}/seek_names.txt".format(fpd_folder_seek)
            fpr_seeknames_file = open(fpr_seeknames_path, "w")
            for sn_item in pr_names_seek:
        ...
        print("\nCreating the filtered protein databases...")
        db_info_dict = {}
        temp_0 = [fpd_fasta_prefix, pr_names_seek, False, None]
        temp_1 = [fpd_fasta_prefix_taxonomy, pr_names_taxonomy, False, None]
        ...
        if pr_names_seek and pr_names_taxonomy:
            log_file_path = "{}/pd_filter_log.txt".format(fpd_folder_seek)
        elif pr_names_seek:
            log_file_path = "{}/pd_filter_log.txt".format(fpd_folder_seek)
    ...
    if pr_names_seek:
        diamond_makedb_version_path = "{}/diamond_makedb_version.txt".format(fpd_folder_seek)
        diamond_makedb_stdoe_path = "{}/diamond_makedb_stdoe.txt".format(fpd_folder_seek)
        process_pr_db(fpd_folder_seek, diamond_db_bash_name, diamond_env, diamond_path, fpd_fasta_seek, fpd_name_seek, diamond_makedb_version_path, diamond_makedb_stdoe_path, thread_num, input_log_file, output_log_file, conda_sh_path)
    if pr_names_taxonomy:
        ...
        process_pr_db(fpr_db_folder_taxonomy, diamond_db_bash_name, diamond_env, diamond_path, fpb_fasta_taxonomy, fpd_name_taxonomy, diamond_makedb_version_path, diamond_makedb_stdoe_path, thread_num, input_log_file, output_log_file, conda_sh_path)
