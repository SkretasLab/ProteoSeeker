...
def basic_stats(br_info_dict, target_info_dict, target_stats_dict, sample_id):
    ...
    # L1 Norm = Sum of the absolute difference of the percentagies of each species found in both of the groups.
    ...
    if (val_sum + pred_sum) != 0:
        second_term = (2 * min_sum) / (val_sum + pred_sum)
        bray_curtis_dis = 1 - second_term
...
def design_metric_grouped_plots(pandas_dict, metric_group_dict, metric_label_dict, legend_label_dict, plot_dir_parh):
    ...
    for key_group_metric in metric_group_dict.keys():
        ...
        legend_props = FontProperties(weight='bold', size=fs_num_2)
        legend_obj = cur_axis.legend(title=legend_title, loc='upper center', bbox_to_anchor=(0.5, -0.2), ncol=5, prop={'size': fs_num_2}, title_fontproperties=legend_props)
        # Replace the legend's labels.
...
def rank_stats(pandas_dict, stats_dir_path):
    ...
    group_min = ["Common species (intersection)", "Unique species from both groups (union)", "Unique to gold group", "Unique to predicted group", "False Positive (FP)", "False Negative (FN)", "L1_norm", "Bray-Curtis Dissimilarity", "Sum of absolute differences", "MAE", "Sum of squared absolute differencies", "MSE", "RMSE"]
...
def plot_full_sample_time(df_full_time_dict, megahit_time_dict, time_dir, stats_dir_path):
    ...
    for key_sample in df_full_time_dict.keys():
        ...
        ax.legend(title=legend_title, loc='center left', bbox_to_anchor=(1, 0.5), prop={'size': fs_num_3}, title_fontproperties=legend_props)
        # Changing the size of the tick labels
...
def plot_total_all_time(total_time_all_df, time_dir, stats_dir_path):
    ...
    total_time_all_barplot.legend.set_title(legend_label, prop={'size': fs_num_2, 'weight': 'bold'})
    # Changing the size of the tick labels        
    plt.tick_params(axis='both', which='major', labelsize=fs_num_3)
    # Layout
    plt.tight_layout()
...
def plot_full_group_sample_time(df_full_time_all, time_dir, stats_dir_path, time_sample_group_dict, time_sample_group_label_dict, methods_group):
    ...
    legend_title = "Stage"
    legend_props = FontProperties(weight='bold', size=fs_num_3)
    cur_axis.legend(title=legend_title, loc='upper center', bbox_to_anchor=(0.5, -0.4), ncol=7, prop={'size': fs_num_3}, title_fontproperties=legend_props)
    # Padding from left, bottom, right, top.
    plt.tight_layout()
    # Space between the plots of the figure.
...
def plot_total_size(df_total_time_dict, sample_size_dict, methods_group, sample_speciesnum_dict, stats_dir_path, time_dir):
    # Font sizes
    fs_num_1 = 20
    fs_num_2 = 15
    fs_num_3 = 15
    ...
    legend_props = FontProperties(weight='bold', size=fs_num_3)
    middle_axis.legend(title=legend_title, loc='upper center', markerscale=2, bbox_to_anchor=(0.5, -0.1), ncol=11, prop={'size': fs_num_3}, title_fontproperties=legend_props)
    # Layout
    # plt.tight_layout()
    plt.tight_layout(rect=[0, 0, 0.99, 0.95])
    # Space between the plots of the figure.
...
def time_analysis(time_dir, stats_dir_path, time_sample_group_dict, time_sample_group_label_dict, sample_size_dict, methods_time_group, sample_speciesnum_dict):
    ...
    plot_total_size(df_total_time_dict, sample_size_dict, methods_time_group, sample_speciesnum_dict, stats_dir_path, time_dir)
...
def benchstats(benchmark_path="12864_2022_8803_MOESM1_ESM.txt", ps_results="", ps_output_dir="analysis_plots_ps", methods_group="k8,k16,k72,k8_ng,k16_ng,k72_ng,cnr,mnr", time_status=True):
    ...
    # It also means that metrics which take into account the percentagies of the predicted species, such as the Bray-Curtis Dissimilarity, will
    # probaby result in "bad" scores when the cases of multiple species for each bin exist.
    # Kraken2
    ...
    metric_group_4 = ["Jaccard Index", "Sorensen-Dice Index", "Bray-Curtis Dissimilarity"]
